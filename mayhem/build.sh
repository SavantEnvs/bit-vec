...
# DWARF<4 first-CU anchor (netnew-fleet-playbook §6): rustc's prebuilt ASan runtime ships
# DWARF-5 and would land at .debug_info offset 0. Link a clang -gdwarf-3 anchor object FIRST
# via a -Clinker cc-wrapper so the first CU is DWARF-3.
...
# Feature-gated suites (serde/borsh/miniserde serialization tests) are enabled so
# the WHOLE upstream suite runs.
echo "=== cargo test --no-run (normal flags, full feature set) ==="
